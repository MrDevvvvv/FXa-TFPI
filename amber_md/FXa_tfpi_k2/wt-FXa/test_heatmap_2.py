...
# Function to extract the numeric part of the second residue for sorting
def extract_second_residue_number(header):
    match = re.search(r':(\d+)', header)
    if match:
        return int(match.group(1))  # Extract numeric part after ':'
    return float('inf')  # Ensure non-numeric values go to the end

# Sort by the second residue number
df = df.sort_index(key=lambda x: x.map(extract_second_residue_number))
