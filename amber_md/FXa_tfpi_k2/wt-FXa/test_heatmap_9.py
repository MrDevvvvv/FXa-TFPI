...
# Determine the maximum number of rows across all heatmaps
max_rows = max(len(indices) for indices in grouped_data.values())

# Define a fixed row height per residue
row_height = 0.4  # Adjust this to change row height
fig_height = max_rows * row_height  # Set a fixed height based on the largest heatmap
...
for second_residue, indices in grouped_data.items():
    ...
    plt.figure(figsize=(12, fig_height))  # Use the same figure height for all heatmaps
