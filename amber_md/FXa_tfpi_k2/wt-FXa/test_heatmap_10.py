...
row_height = 0.4  # Adjust this to set uniform row height

# Determine the max number of rows across all heatmaps
max_rows = max(len(indices) for indices in grouped_data.values())

# Set a **fixed height** for all heatmaps
fig_height = max_rows * row_height  # Every heatmap will have this height
...
for second_residue, indices in grouped_data.items():
    ...
    plt.figure(figsize=(12, fig_height))  # Use a fixed figure height for all heatmaps
    ax = sns.heatmap(group_df, cmap='YlGnBu', cbar=True, linewidths=0, cbar_kws={"ticks": [0, 10]})

    # Modify y-axis labels to show only the first residue (e.g., "LYS-147")
    y_labels = [extract_residues(res)[0] for res in group_df.index]

    # Set y-ticks in the middle of each row
    y_ticks = [tick + 0.5 for tick in range(len(y_labels))]  # Adjust positions
    plt.yticks(ticks=y_ticks, labels=y_labels, rotation=0, verticalalignment='center')
