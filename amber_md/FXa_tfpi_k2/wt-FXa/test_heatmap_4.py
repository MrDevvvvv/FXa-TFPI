...
# Function to extract second residue identifier (e.g., ARG-244 from LYS-47:ARG-244)
def extract_second_residue(header):
    parts = header.split(':')
    if len(parts) == 2:
        return parts[1]  # Return second residue (e.g., ARG-244)
    return "Unknown"

# Group by second residue type
second_residue_groups = df.groupby(by=extract_second_residue)

# Generate separate heatmaps for each second residue type
for second_residue, group_df in second_residue_groups:
    ...
    x_ticks = list(range(0, group_df.shape[1], 5))  # Generate ticks at intervals of 5
